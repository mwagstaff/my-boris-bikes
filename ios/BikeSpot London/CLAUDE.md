...
This is a native iOS app called "BikeSpot London" built with SwiftUI and Swift 5.0, targeting iOS 18.5+. The project is created with Xcode and follows standard iOS development patterns.
...
- `xcodebuild -scheme "BikeSpot London" -configuration Debug build` - Build the app for development
- `xcodebuild -scheme "BikeSpot London" -configuration Release build` - Build for release
- Use Xcode IDE to run on simulator or device

### Testing
- `xcodebuild test -scheme "BikeSpot London"` - Run all tests
- `xcodebuild test -scheme "BikeSpot London" -destination 'platform=iOS Simulator,name=iPhone 15'` - Run tests on specific simulator
...
### Project Structure
- `BikeSpot London/` - Main app source code
  - `BikeSpot_LondonApp.swift` - App entry point with `@main` attribute
  - `ContentView.swift` - Root SwiftUI view
  - `Assets.xcassets/` - App icons and assets
- `BikeSpot LondonTests/` - Unit tests using Swift Testing framework
- `BikeSpot LondonUITests/` - UI tests for the application
...
- Import the main module with `@testable import BikeSpot_London`