...
GRAFANA_URL="https://api.skynolimit.dev/grafana"
API="$GRAFANA_URL/api"

GRAFANA_USER="${GRAFANA_USER:-admin}"
GRAFANA_PASSWORD="${GRAFANA_PASSWORD:?GRAFANA_PASSWORD must be set}"

FOLDER_UID="${FOLDER_UID:-ffcqxdxacxlogf}" # "My Boris Bikes"
FOLDER_TITLE="${FOLDER_TITLE:-my-boris-bikes}"
PROM_DS_NAME="${PROM_DS_NAME:-Prometheus}"
PROM_DS_UID="${PROM_DS_UID:-my-boris-bikes-prometheus}"
...
PROM_SCRAPE_JOB_NAME="${PROM_SCRAPE_JOB_NAME:-my-boris-bikes-api}"
PROM_SCRAPE_TARGET="${PROM_SCRAPE_TARGET:-host.docker.internal:3010}"
...
upsert_prometheus_scrape_config_file() {
  local config_file="$1"
  local begin_marker end_marker block tmp_file escaped_target
...
EOF
  ...
  # Normalize any host-based scrape target on port 3010 to the selected target.
  escaped_target="${PROM_SCRAPE_TARGET//\\/\\\\}"
  escaped_target="${escaped_target//&/\\&}"
  sed -E -i \
    "s#(host\\.docker\\.internal|localhost|[0-9]{1,3}(\\.[0-9]{1,3}){3}):3010#${escaped_target}#g" \
    "$config_file"
